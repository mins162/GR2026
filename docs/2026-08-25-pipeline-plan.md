...
| --- | --- | --- | --- |
| ~~S1 route ∥ S2 preprocessing~~ | **불가 판정 (2026-08-26)** — `rsmt`는 clear되지 않아 S2 재구성 루프는 no-op. 실제 2s는 전부 `generate_detours`인데 입력(of_nets, congestion view)이 모두 S1-최종이라 파이프라인 지점이 없음. 투기(중간 congestion으로 선계산)만 가능하나 품질 리스크·재사용률 불확실 | ~1.8 / 7.1s | "가장 안전" 판정은 RSMT 재구성이 실재한다는 전제였음 — 철회 |
