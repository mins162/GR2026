...
## 6. 측정 환경 주의 — 공용 CPU 영향

- FLUTE의 CPU 시간 안에 GPU 시간이 숨는 구조인데, 지금 측정은 **CPU가 많이 느린 상태**에서 나온 값
- 현재 `gpu-5` 서버는 GPU만 쓰고 **CPU는 공용**인데, 논문 시즌이라 CPU가 바쁨
- 인턴용 서버는 **개별 CPU**를 붙여준다고 하므로, 배정받으면 다시 측정 예정 — CPU 병목 항목(FLUTE, DAG 구성, input 파싱 등) 수치가 달라질 수 있음
