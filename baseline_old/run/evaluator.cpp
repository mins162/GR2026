...
bool NVR_DB::read_gr_solution(const char *input)
{
  ...
  for(unsigned z = 0; z < m_graph.num_layer(); z++) {
    ...
    for(unsigned x = 0; x < m_graph.num_gridx(); x++) {
      for(unsigned y = 0; y < m_graph.num_gridy(); y++) {
        ...
        //if(z == 1) printf("(%d, %d): %.1f\n", x, y, demand * 0.5);
        //if(z == 1 && x == 14 && y == 226) std::cerr << "WIRE/VIA COUNT " << wire_counter[z][x][y] << ' ' << stacked_via_counter[z][x][y] << std::endl;
        //if(z == 1 && x == 24 && y == 297) printf("WIRE COUNT = %d VIA COUNT = %d\n", wire_counter[z][x][y], stacked_via_counter[z][x][y]);
    ...
    printf("Layer = %d, layer overflow cost = %lf (unit cost = %.2f)\n", z, layer_overflows, gg.unit_overflow_cost());
  ...
  printf("FOR_STAT %.0f %.0f %.0f %.0f\n", wl_cost, via_cost, overflow_cost, total_cost);
  return true;
}
  ...
  std::vector< std::vector< std::vector<int> > > &stacked_via_counter) const
{
  for(const NVR_Point3D &pp : via_loc) {
    if(flag[pp.z()][pp.x()][pp.y()] != net_idx) {
      //if(pp.z() == 1 && pp.x() == 11 && (pp.y() == 268 || pp.y() == 269))
      //  std::cerr << "NET NAME " << m_nets[net_idx].m_name << std::endl << pp.z() << ' ' << pp.x() << ' ' << pp.y() << std::endl;
