...
def build():
    ...
    fig, ax = plt.subplots(figsize=(4.32, 2.13))
    ...
    ax.text(d["gpu"] + d["cpu"] + 0.15, 2.87, "%.2f s" % (d["gpu"] + d["cpu"]),
            va="center", fontsize=5.7, weight="bold")
    ...
    ax.text(d["overlapped_cpu"] + 0.15, 1.62, "%.2f s" % d["overlapped_cpu"],
            va="center", fontsize=5.7, weight="bold", color=style.OURS)
    ...
    ax.set_yticklabels(["serial: GPU then CPU", "overlapped: CPU lane",
                        "overlapped: GPU lane"], fontsize=5.2)
