...
def build():
    ...
    for i, (name, base, opt, mult) in enumerate(rows):
        ax.text(i - w / 2, base + 1.5, "%.2f" % base, ha="center", fontsize=6, color=style.MUTED)
        ax.text(i + w / 2, opt + 1.5, "%.2f" % opt, ha="center", fontsize=6, color=style.OURS)
        ax.text(i, max(base, opt) + 6.5, "%.2fx" % mult, ha="center",
                fontsize=8, weight="bold", color=style.INK)
