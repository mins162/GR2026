...
def build():
    fig, ax = plt.subplots(figsize=(4.32, 2.35))
    ...
    ax.text(left_base + 1.5, 1, "92.47 s", va="center", fontsize=5.7, weight="bold")
    ax.text(left_opt + 1.5, 0, "28.28 s", va="center", fontsize=5.7, weight="bold",
            color=style.OURS)
    ax.set_yticks([1, 0])
    ax.set_yticklabels(["base", "opt"], fontsize=6.2)
    ax.set_xlabel("runtime (s), mempool_group")
    ax.set_xlim(0, 104)
    ax.set_ylim(-0.5, 1.5)
    ax.yaxis.grid(False)
    ax.legend(fontsize=4.7, ncol=3, loc="lower center", bbox_to_anchor=(0.5, -0.50))
    style.strip(ax)
    return fig
